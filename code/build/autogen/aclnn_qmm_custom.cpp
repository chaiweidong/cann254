...
#include "QmmCustom_op_resource.h"
...
[[maybe_unused]] uint32_t NNOPBASE_QmmCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
...
#ifdef ACLNN_WITH_BINARY
    static uint32_t QmmCustomOpTypeId = op::GenOpTypeId("QmmCustom", QmmCustom_RESOURCES);
#endif
    static NnopbaseDfxId dfxId = {0x60000, __func__, false};
    static NnopbaseDfxId tilingId = {0x60000, "aclnnQmmCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "QmmCustom";
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
...
#ifdef ACLNN_WITH_BINARY
    NnopbaseAddOpTypeId(*executor, QmmCustomOpTypeId);
#endif
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, x1, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, x2, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, scale, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, pertokenScaleOptional, 3));
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "scale", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 3, "pertokenScaleOptional", true));
